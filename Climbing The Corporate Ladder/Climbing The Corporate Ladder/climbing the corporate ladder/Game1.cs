    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            else if (gameState == GameStates.Game)
            {
                ...
                // new level
                if (player.Location.X > 6200)
                {
                    player.Location = new Vector2(0, 350);
                    World.viewport.X = 0;
                    count = 0;
                    player.Life = 10;
                }
            ...
            if (gameState == GameStates.Game)
            {
                ...
                if (timer <= 60)
                {
                    ...
                    if (timer > penCD)
                    {
                        ...
                        if (kbd.IsKeyDown(Keys.R) && player.currentAnimation != "run")
                        {
                ...
                foreach (Object obj in enemies)
                {
                    ...
                    if (rekt.Intersects(new Rectangle(rect.X + 100, rect.Y, rect.Width, rect.Height)) && isPunch == false)
                    {
                        player.Life -= 1;
                        deadIndex = countIndex;

                    }
                    if (rekt.Intersects(new Rectangle(rect.X + 100, rect.Y, rect.Width, rect.Height)) && isPunch == true)
                    {
                        deadIndex = countIndex;
                        killed++;
                    }
                    if ((penVisible) && penHitBox.Intersects(new Rectangle(rect.X + 100, rect.Y, rect.Width, rect.Height)))
                    {
                        penVisible = false;
                        deadIndex = countIndex;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            if (gameState == GameStates.Game)
            {
                ...
                //construction of testing values - not needed
